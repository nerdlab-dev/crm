...
const SERVER_NAME = "nerdboard-crm";
...
const COMMANDS = {
  codex: {
    ...
    login: `codex mcp login --scopes ${CRM_SCOPES} ${SERVER_NAME}`,
  },
  claude: {
    get: ["mcp", "get", SERVER_NAME],
    add: [
...
export function installMcp({
  ...
  if (current.status === 0) {
    if (current.stdout.includes(REMOTE_URL)) {
      return {
        ok: true,
        message: `Nerdboard CRM MCP is already connected to ${client}.\n${commands.login}`,
      };
    }
  ...
  return {
    ok: true,
    message: `Connected Nerdboard CRM MCP to ${client}.\n${commands.login}`,
  };
}
